...
typedef struct {
        RetVal conflict_tag=RetVal::r_normal;
        int w1=0;
        int w2=0;
        int unit=0;
        // int antecedent=-1;
...
typedef struct {
        vector<int> affteced_clause;
        bool returen_switch=false;
        int tag=-1;
}twpresult;
...
private:
  ...
  twpresult two_watch_propogate(int,int);
  bool existence(int);
  csresult check_status(vector<int>,int,int);
  // vector<int> first_literal_watch;
  // vector<int> second_literal_watch;
  vector<vector<int>> literal_watch;
  vector<vector<int>> clauses_literal_watched;


public:
  SATSolverCDCL() : generator(random_generator()) {} // constructor
  void initialize(string path);                                 // to initialize the solver
 ...
 */
void SATSolverCDCL::initialize(string path) {
  ...
  // first_literal_watch.clear();
  // first_literal_watch.resize(literal_count, -1);  // -1: empty
  // second_literal_watch.clear();
  // second_literal_watch.resize(literal_count, -1);  // -1: empty
  literal_watch.clear();
  literal_watch.resize(literal_count); 
  clauses_literal_watched.clear();
  clauses_literal_watched.resize(clause_count);
...
int SATSolverCDCL::CDCL() {
  ...
  while (!all_variables_assigned()) {
     ...
     */
    twpresult twp_result = two_watch_propogate(picked_variable,decision_level);
    while (twp_result.returen_switch==false) {
      decision_level = conflict_analysis_and_backtrack(decision_level);
      int variable_next=unit_queue.back();
      unit_queue.pop_back();
      assign_literal(variable_next,decision_level,literal_antecedent[literal_to_variable_index(variable_next)]);
      twpresult twp_result = two_watch_propogate(variable_next,decision_level);
...
void SATSolverCDCL::create_watchList(){
  int literal_index;
  int first_watch_literal;
  int second_watch_literal;
  for (int i = 0; i < literal_list_per_clause.size(); i++){
    int first = 0;
    // vector<int> two_watch_literals;
    // two_watch_literals.resize(2);
    for (int j = 0; j < literal_list_per_clause[i].size(); j++) {
      literal_index =
            literal_to_variable_index(literal_list_per_clause[i][j]);
        if (literals[literal_index] == -1 && first==0) 
        {
          first_watch_literal = literal_list_per_clause[i][j];
          first=1;
          continue;
        }
        if (literals[literal_index] == -1 && first==1) 
        {
          second_watch_literal = literal_list_per_clause[i][j];
          break;
        }
    clauses_literal_watched[i].push_back(literal_to_variable_index(first_watch_literal));
    clauses_literal_watched[i].push_back(literal_to_variable_index(second_watch_literal));
    // first_literal_watch[literal_to_variable_index(first_watch_literal)] = i;
    // second_literal_watch[literal_to_variable_index(second_watch_literal)] = i;
    literal_watch[literal_to_variable_index(first_watch_literal)].push_back(i);
    literal_watch[literal_to_variable_index(second_watch_literal)].push_back(i);
    }
  }
}

twpresult SATSolverCDCL::two_watch_propogate(int variable, int decision_level){
  twpresult twp_result;
  vector<int> prop_list;
  prop_list.push_back(variable);
  while (prop_list.size()!=0){
    int variable = prop_list.back();
    prop_list.pop_back();
    for(int i=0; literal_watch.size(); i++){
      for (int j=0; literal_watch[i].size(); j++){
        int affected_clause_num = literal_watch[i][j];
        vector<int> affected_clause = literal_list_per_clause[affected_clause_num];
        int first_watch_literal=clauses_literal_watched[affected_clause_num][0];
        int seconod_watch_literal=clauses_literal_watched[affected_clause_num][1];
        int first_prev=first_watch_literal;
        int second_prev=seconod_watch_literal;
        csresult status_result=check_status(affected_clause,first_watch_literal,seconod_watch_literal);
        if(status_result.conflict_tag=RetVal::r_unit){
          prop_list.push_back(status_result.unit);
          literal_antecedent.push_back(literal_watch[i][j]);
          unit_queue.push_back(i);
        }
        else if(status_result.conflict_tag=RetVal::r_unsatisfied){
          twp_result.affteced_clause=affected_clause;
          return twp_result;
        }
        literal_watch[first_prev].erase(literal_watch[first_prev].begin()+affected_clause_num-1);
        literal_watch[second_prev].erase(literal_watch[second_prev].begin()+affected_clause_num-1);
        clauses_literal_watched[affected_clause_num][0] = status_result.w1;
        clauses_literal_watched[affected_clause_num][1] = status_result.w2;
        literal_watch [status_result.w1].push_back(affected_clause_num);
        literal_watch [status_result.w2].push_back(affected_clause_num);
      }
      
    }
  }
  twp_result.returen_switch=true;
  return twp_result;
}

csresult SATSolverCDCL::check_status(vector<int> clause,int watch_literal1,int watch_literal2){
  csresult result;
  int unit=0;
  int index1=literal_to_variable_index(watch_literal1);
  int index2=literal_to_variable_index(watch_literal2);
  if(existence(literals[index1])||existence(literals[index2])){
    result.conflict_tag=RetVal::r_satisfied;
    result.w1=watch_literal1;
    result.w2=watch_literal2;
    result.unit=unit;
    return result;
  }
  vector<int> sym;
  for(int i=0;i<clause.size();i++){
    ...
    if(existence(-clause[i])==false){
      sym.push_back(clause[i]);
    }
    if(existence(clause[i]==true)){
      if(existence(-watch_literal1)==false){
        result.conflict_tag=RetVal::r_satisfied;
        result.w1=watch_literal1;
        result.w2=clause[i];
        result.unit=unit;
  ...
  if(sym.size()==1){
    result.conflict_tag=RetVal::r_unit;
    result.w1=watch_literal1;
    result.w2=watch_literal2;
    result.unit=sym[0];
    return result;
    ...
    result.conflict_tag=RetVal::r_unsatisfied;
    result.w1=watch_literal1;
    result.w2=watch_literal2;
    result.unit=unit;
    return result;
    ...
    result.conflict_tag=RetVal::r_unresolved;
    result.w1=sym[0];
    result.w2=sym[1];
    result.unit=unit;
    return result;
...
bool SATSolverCDCL::existence(int varialbe){
  int temp_index=literal_to_variable_index(varialbe);
  int value_variable=varialbe>0?1:0;
  int value_variable_current=literals[temp_index];
  if(value_variable==value_variable_current) return true;
  else return false;
}
...
int main( int argc, char *argv[])  
{
  ...
  // if( argc == 2 ){
  string file_name="ii8a1";
  // cout<<"The argument supplied is "<<argv[1]<<endl;
  // string file_name=argv[1];
  string base_path="C:\\Users\\femst\\Workplace\\Documents_c++\\SAT_project3\\sat\\";
  string file_path=base_path+file_name+".cnf";
  cout<<file_path<<endl;
